...
After you run `/speckit.tasks` in your spec-kit project, `beadsync` picks up the generated `tasks.md`, creates real [Beads](https://github.com/steveyegge/beads) in the local `bd` database, and pushes them to GitHub Issues — with full idempotency, project scoping, and a clear source-of-truth model.
...
/speckit.tasks  (run inside your spec-kit project)
        │
        ▼
specs/<NNN-feature>/tasks.md            ← generated by spec-kit (default)
.specify/specs/<NNN-feature>/tasks.md   ← alternate layout (also supported)
        │
        ▼
beadsync speckit-to-beads               ← Phase 1  (auto-discovers tasks.md)
        │
        ▼
  bd (Bead database)                    ← Source of truth after this point
        │
        ▼
beadsync sync --repo owner/repo         ← Phase 2
        │
        ▼
...
After this, `beadsync` works from any directory.
...
beadsync sync --repo owner/my-project
```

That's it. Re-run `beadsync sync` whenever you update tasks — unchanged Beads are skipped automatically.
...
```bash
beadsync speckit-to-beads [options]
```

Auto-discovers all `tasks.md` files under `specs/*/tasks.md` and `.specify/specs/*/tasks.md` in the current directory and creates or updates Beads in the local `bd` database. No file path argument needed. If the same file appears in both trees it is processed only once.
...
beadsync speckit-to-beads --project my-project
```
...
spec-kit generates `tasks.md` under `specs/<NNN-feature-name>/tasks.md` at the repo root (this is the default). Some setups use `.specify/specs/<NNN-feature-name>/tasks.md` instead — `beadsync` checks both locations automatically. Each task is a markdown checklist line:
...
beadsync sync --repo acme/my-project --dry-run
...
beadsync speckit-to-beads               # picks up new/changed tasks
beadsync sync --repo acme/my-project    # syncs changes to GitHub
```
...
`beadsync` automatically exports `.beads/issues.jsonl` after every write command. This JSONL file is one bead per line — plain text, git-diffable, and auto-mergeable.
...
beadsync speckit-to-beads                # auto-exports issues.jsonl
...
```html
<!-- beadsync:external_id:speckit-my-project-T001 -->
```
...
```
beadsync                       ← the CLI script (single file, v3.0.0)
examples/
