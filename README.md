...
/speckit.tasks  (run inside your spec-kit project)
        │
        ▼
.specify/specs/<NNN-feature>/tasks.md   ← generated by spec-kit
        │
        ▼
...
Auto-discovers all `tasks.md` files under `.specify/specs/*/tasks.md` in the current directory and creates or updates Beads in the local `bd` database. No file path argument needed.
...
spec-kit generates `tasks.md` under `.specify/specs/<NNN-feature-name>/tasks.md`. Each task is a markdown checklist line:
...
#   /speckit.tasks
# This creates: .specify/specs/001-my-feature/tasks.md
